...
/**
  * HTML: <canvas id="canvas" style="display:none"></canvas>
          <img src="ajdg.jpg" id="compressedimg">
  * Js  :
...
(function(global){
   ...
   function Compress(canvasId){
	    this.stage=stage=new createjs.Stage(canvasId);
		...
		this.isIphoneOs = sUserAgent.match(/iphone os/i) == "iphone os"; 

   }
   
   
   Compress.prototype.init=function(canvasId){
		var _this=this;
   ...
   Compress.prototype.reduce=function(imgid,ratio){
	     var _this=this;
         ...
         EXIF.getData(newimg, function(){
            ori=EXIF.getAllTags(this).Orientation||0;
         });
		 _this.stage.removeChild(_this.bitmap);
	     width=canvas.width = img.naturalWidth;	
		 height=canvas.height =img.naturalHeight;
         _this.bitmap=new createjs.Bitmap(img.src); 
		 _this.bitmap.regX = 0, _this.bitmap.regY =0, _this.bitmap.x =0, _this.bitmap.y =0; //绕左上角转90度 在向x轴正方形移动宽度
		 
		 if((ori==6||ori==8)&&_this.isIphoneOs){
		     _this.bitmap.rotation=90;
		     _this.bitmap.x=width;
		 }
		 		

		 _this.stage.addChild(_this.bitmap); 
		 _this.stage.update();
         if((width*height)<500000){ratio=ratio*5; if(ratio>1){ratio=1}}
         var newImageData = _this.canvas.toDataURL(mime_type, ratio);
         return newImageData;        
   }
